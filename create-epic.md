...
- `<parent-issue-key>`: Parent initiative key (e.g., UKCAUD-909)

**Examples**:

```bash
/create-epic ukcaud "User Onboarding Workflows" UKCAUD-909
/create-epic ukcas "Checklist Automation Engine" UKCAS-920
/create-epic dist "EMEA Localization Testing" DIST-607
```
...
Parent initiative: Which initiative does this epic support? (provide key, e.g., UKCAUD-909)
> UKCAUD-909

Description: What's being built? (2-3 sentences)
> Build guided workflows for first-time users. Includes onboarding checklist, tutorial mode, and contextual help. Reduces time to first successful query from 10 minutes to 2 minutes.
...
if (!title || !parentKey || !description) {
  throw new Error(
    "Missing required fields: title, parent key, and description are required",
  );
}

if (!/^(UKCAUD|UKJPD|UKCAS|DIST)-\d+$/.test(parentKey)) {
  throw new Error(
    "Parent key must be a valid Jira issue key (e.g., UKCAUD-909)",
  );
}
```
...
if (!parent) {
  throw new Error(
    `❌ Parent issue ${parentKey} does not exist. Please verify the issue key and try again.`,
  );
}

if (parent.fields.issuetype.name !== "Initiative") {
  throw new Error(
    `❌ Parent ${parentKey} is type "${parent.fields.issuetype.name}", not "Initiative". Epics must have Initiative parents.`,
  );
}
...
#### 2.3 Check Parent is in Valid Project

```javascript
const validProjects = ["UKCAUD", "UKJPD", "UKCAS", "DIST"];
if (!validProjects.includes(parent.fields.project.key)) {
  throw new Error(
    `Parent ${parentKey} is in project ${parent.fields.project.key}, not a valid UK project. Epics must be in same project as parent.`,
  );
}
```
...
const duplicates = await mcp__atlassian__jira_search_issues({
  jql: `project = ${parent.fields.project.key} AND issuetype = Epic AND parent = ${parentKey} AND summary ~ "${title}"`,
  maxResults: 5,
});
...
if (parent.fields.duedate) {
  ...
  if (targetDateObj > parentDueDate) {
    ...
    const proceed = await promptUser(
      "Epic date extends beyond parent. Proceed anyway? (yes/no)",
    );
    if (proceed !== "yes") {
      throw new Error("Epic creation cancelled by user");
    }
...
```javascript
const labelMap = {
  ukcaud: ["CWAS-Feature", "UKCAUD", "UKCAUD_Epic_DoR"],
  ukcas: ["CWAS-Support", "UKCAS", "UKCAUD_Epic_DoR"],
  dist: ["CWAS-Distribution", "DIST", "UKCAUD_Epic_DoR"],
  ukjpd: ["CWAS-Discovery", "UKJPD", "UKCAUD_Epic_DoR"],
};

const labels = labelMap[initiativeName] || ["UKCAUD_Epic_DoR"];
```
...
```javascript
const createdEpic = await mcp__atlassian__jira_get_issue({
  issueKey: issueKey,
});
...
if (!parentPageId) {
  console.warn(
    "⚠️  Could not find parent Confluence page ID from Jira description",
  );
  console.warn("   Epic page will be created under Initiatives parent instead");
  parentPageId = "[VERIFY: Confluence Initiatives parent page ID for UK space]"; // Fallback
}
          ...
          marks: [
            {
              type: "link",
              attrs: {
                href: `https://caseware.atlassian.net/browse/${parentKey}`,
              },
            },
          ],
        },
...
```javascript
const epicsPath = `C:/Users/liam.bond/Documents/Productivity Tool/workspace/initiatives/${initiativeName}/epics.md`;
...
- Coordinate with UK team if needed
`;
...
6. Coordinate with UK team if needed
...
You can search for initiatives with:
  JQL: project in (UKCAUD, UKJPD, UKCAS, DIST) AND issuetype = Initiative
...
If you intended to create a Story under an Epic, use:
  project.key = valid UK project
  issuetype = "Story"
  parent = ${parentKey}

Otherwise, provide a valid Initiative key (e.g., UKCAUD-909, UKCAS-920).
```
...
⚠️  Potential duplicate epics found under ${parentKey}:
   - UKCAUD-897: User Feedback Logging & Engagement Quality
   - UKCAUD-606: UK Engagement Intelligence Alpha
...
- [ ] Parent key provided and in format UKCAUD-XXX / UKCAS-XXX / etc.
...
- [ ] Parent issue is in valid UK project (UKCAUD, UKJPD, UKCAS, DIST)
...
### Example 1: UKCAUD Epic Under UKCAUD-909

```bash
/create-epic ukcaud "User Feedback Logging" UKCAUD-909

Description: Build feedback infrastructure for engagement quality tracking. Users provide feedback on responses with full conversation trace.
Target date: 2026-03-15

✅ Validated parent: UKCAUD-909 (UK Delivery Initiative)
✅ Created Jira epic: UKCAUD-1237
✅ Verified parent linkage: UKCAUD-1237 → UKCAUD-909
✅ Updated initiatives/ukcaud/epics.md
✅ Logged decision in initiatives/ukcaud/context.md

Next: Create Confluence epic one-pager from template [VERIFY: Confluence template ID for UK space]
```

### Example 2: UKCAS Epic Under UKCAS-920
...
✅ Validated parent: UKCAS-920 (UKCAS Alpha)
✅ Created Jira epic: UKCAS-1238
✅ Verified parent linkage: UKCAS-1238 → UKCAS-920
✅ Updated initiatives/ukcas/epics.md
✅ Logged decision in initiatives/ukcas/context.md

Next: Create Confluence epic one-pager from template [VERIFY: Confluence template ID for UK space]
```

### Example 3: DIST Epic Under DIST-607

```bash
/create-epic dist "EMEA Localization Testing" DIST-607

Description: Build testing infrastructure for EMEA region localization. Includes embeddings localization, chat regional testing, and compliance validation.
Target date: 2026-06-01

✅ Validated parent: DIST-607 (UK Regional Rollout)
✅ Created Jira epic: DIST-1239
✅ Verified parent linkage: DIST-1239 → DIST-607
✅ Updated initiatives/dist/epics.md
